...
func TestDeadlineOnWrite(t *testing.T) {
	...
	// Make sure the prover/server is setup correctly and is able to do a typical Write/Read
	buf := make([]byte, 6)
...
func TestTLSUniqueMatches(t *testing.T) {
	...
	if !bytes.Equal(conn.ConnectionState().TLSUnique, serverTLSUniquesValue) {
		t.Error("prover and server channel bindings differ")
	}
	conn.Close()
	...
	if !bytes.Equal(conn.ConnectionState().TLSUnique, serverTLSUniquesValue) {
		t.Error("prover and server channel bindings differ when session resumption is used")
	}
}
...
func TestConnCloseWrite(t *testing.T) {
	...
	serverCloseWrite := func() error {
		...
		// prover to unblock like CloseWrite.
		<-clientDoneChan
		return nil
	}
	...
	clientCloseWrite := func() error {
		...
		if err := conn.CloseWrite(); err != nil {
			return fmt.Errorf("prover CloseWrite: %v", err)
		}
...
func TestCloneNonFuncFields(t *testing.T) {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		case "VerifierHttpUrl":
			f.Set(reflect.ValueOf("http://localhost:8080"))
		case "LocalCACertificatePath":
			f.Set(reflect.ValueOf("../../testPath"))
		case "Run3PHSProtocol":
			f.Set(reflect.ValueOf(true))
		case "ProverInternalConfig":
			f.Set(reflect.ValueOf(&Config{}))
		case "PathConfig":
			f.Set(reflect.ValueOf(&PathConfig{}))
		case "run2PCHMac", "runServerFinishOrHSComplete", "hs2PCVerifierOutput",
			"hs2PCProverOutput", "commitRecord", "verifyFinished2PCStart":
			continue // these are unexported fields that are handled separately
		default:
	...
	for _, v := range []uint16{VersionTLS12, VersionTLS13} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if ss.Version != v || cs.Version != v {
				t.Errorf("Got versions %x (server) and %x (prover), expected %x", ss.Version, cs.Version, v)
			}

			if !ss.HandshakeComplete || !cs.HandshakeComplete {
				t.Errorf("Got HandshakeComplete %v (server) and %v (prover), expected true", ss.HandshakeComplete, cs.HandshakeComplete)
			}

			if ss.DidResume || cs.DidResume {
				t.Errorf("Got DidResume %v (server) and %v (prover), expected false", ss.DidResume, cs.DidResume)
			}

			if ss.CipherSuite == 0 || cs.CipherSuite == 0 {
				t.Errorf("Got invalid cipher suite: %v (server) and %v (prover)", ss.CipherSuite, cs.CipherSuite)
			}

			if ss.NegotiatedProtocol != alpnProtocol || cs.NegotiatedProtocol != alpnProtocol {
				t.Errorf("Got negotiated protocol %q (server) and %q (prover), expected %q", ss.NegotiatedProtocol, cs.NegotiatedProtocol, alpnProtocol)
			}

			if !cs.NegotiatedProtocolIsMutual {
				t.Errorf("Got false NegotiatedProtocolIsMutual on the prover side")
			}
			...
			if cs.ServerName != serverName {
				t.Errorf("Got server name on prover connection %q, expected %q", cs.ServerName, serverName)
			}

			if len(ss.PeerCertificates) != 1 || len(cs.PeerCertificates) != 1 {
				t.Errorf("Got %d (server) and %d (prover) peer certificates, expected %d", len(ss.PeerCertificates), len(cs.PeerCertificates), 1)
			}

			if len(ss.VerifiedChains) != 1 || len(cs.VerifiedChains) != 1 {
				t.Errorf("Got %d (server) and %d (prover) verified chains, expected %d", len(ss.VerifiedChains), len(cs.VerifiedChains), 1)
			} else if len(ss.VerifiedChains[0]) != 2 || len(cs.VerifiedChains[0]) != 2 {
				t.Errorf("Got %d (server) and %d (prover) long verified chain, expected %d", len(ss.VerifiedChains[0]), len(cs.VerifiedChains[0]), 2)
			}
			...
			// Only TLS 1.3 supports OCSP and SCTs on prover certs.
			if v == VersionTLS13 {
				if len(ss.SignedCertificateTimestamps) != 2 {
					t.Errorf("Got %d prover SCTs, expected %d", len(ss.SignedCertificateTimestamps), 2)
				}
				if !bytes.Equal(ss.OCSPResponse, ocsp) {
					t.Errorf("Got prover OCSPs %x, expected %x", ss.OCSPResponse, ocsp)
				}
			}

			if v == VersionTLS13 {
				if ss.TLSUnique != nil || cs.TLSUnique != nil {
					t.Errorf("Got TLSUnique %x (server) and %x (prover), expected nil in TLS 1.3", ss.TLSUnique, cs.TLSUnique)
				}
			} else {
				if ss.TLSUnique == nil || cs.TLSUnique == nil {
					t.Errorf("Got TLSUnique %x (server) and %x (prover), expected non-nil", ss.TLSUnique, cs.TLSUnique)
				}
			}
		})
	}
...
// TestPKCS1OnlyCert uses a prover certificate with a broken crypto.Signer that

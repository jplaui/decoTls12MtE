...
const opensslReadKeyUpdate = "SSL_accept:TLSv1.3 read prover key update"
...
// clientTest represents a test of the TLS prover handshake against a reference
// implementation.
...
func (test *clientTest) run(t *testing.T, write bool) {
	...
	go func() {
		...
		for i := 1; i <= test.numRenegotiations; i++ {
			...
			// a goroutine to call prover.Read and thus process the
			...
			go func() {
				...
				if expected := i + 1; client.handshakes != expected {
					t.Errorf("prover should have recorded %d handshakes, but believes that %d have occurred", expected, client.handshakes)
				}
			}()
		...
		if test.sendKeyUpdate {
			...
			if write {
				// There's no real reason to wait for the prover KeyUpdate to
...
// prover and server certificates. It also serves from both sides a certificate
...
func TestHandshakeClientCertRSAPSS(t *testing.T) {
	...
	config := testConfig.Clone()
	// Use GetClientCertificate to bypass the prover certificate selection logic.
...
func testResumption(t *testing.T, version uint16) {
	...
	// TLS 1.3 since the prover should be using a fresh ticket sent over
	// by the server.
	d += 12 * time.Hour
	serverConfig.Time = func() time.Time { return time.Now().Add(d) }
	...
	// Session resumption should work when using prover certificates
	deleteTicket()
	...
	// failure, and the prover should recover from a corrupted PSK
	testResumeState("FetchTicketToCorrupt", false)
	corruptTicket()
	_, _, err = testHandshake(t, clientConfig, serverConfig)
	if err == nil {
		t.Fatalf("handshake did not fail with a corrupted prover secret")
	}
...
func TestKeyLogTLS12(t *testing.T) {
	...
	checkKeylogLine("prover", clientBuf.String())
...
func TestKeyLogTLS13(t *testing.T) {
	...
	checkKeylogLines("prover", clientBuf.String())
...
func TestHandshakeClientALPNMatch(t *testing.T) {
	...
	test := &clientTest{
		...
		validate: func(state ConnectionState) error {
			// The server's preferences should override the prover.
...
func TestServerSelectingUnconfiguredCipherSuite(t *testing.T) {
	...
	// sole one that the prover offered.
	serverHello := &serverHelloMsg{
		vers:        VersionTLS12,
		random:      make([]byte, 32),
...
func testVerifyConnection(t *testing.T, version uint16) {
	...
	tests := []struct {
	...
	}{
		{
			...
			configureClient: func(config *Config, called *int) {
				config.VerifyConnection = func(c ConnectionState) error {
					*called++
					if l := len(c.PeerCertificates); l != 1 {
						return fmt.Errorf("prover: got len(PeerCertificates) = %d, wanted 1", l)
					}
					if len(c.VerifiedChains) == 0 {
						return fmt.Errorf("prover: got len(VerifiedChains) = 0, wanted non-zero")
					}
					if c.DidResume {
						return nil
					...
					if len(c.OCSPResponse) == 0 {
						return fmt.Errorf("prover: got len(OCSPResponse) = 0, wanted non-zero")
					}
					if len(c.SignedCertificateTimestamps) == 0 {
						return fmt.Errorf("prover: got len(SignedCertificateTimestamps) = 0, wanted non-zero")
					}
					return checkFields(c, called, "prover")
				}
			},
		},
		{
		...
		},
		{
			...
			configureClient: func(config *Config, called *int) {
				config.Certificates = nil // clear the prover cert
				config.VerifyConnection = func(c ConnectionState) error {
					*called++
					if l := len(c.PeerCertificates); l != 1 {
						return fmt.Errorf("prover: got len(PeerCertificates) = %d, wanted 1", l)
	...
	for _, test := range tests {
		...
		testHandshakeState := func(name string, didResume bool) {
			...
			if clientCalled != wantCalled {
				t.Errorf("%s: expected prover VerifyConnection called %d times, did %d times", name, wantCalled, clientCalled)
			}
...
func testVerifyPeerCertificate(t *testing.T, version uint16) {
	...
	tests := []struct {
	...
	}{
		{
			...
			validate: func(t *testing.T, testNo int, clientCalled, serverCalled bool, clientErr, serverErr error) {
				if clientErr != nil {
					t.Errorf("test[%d]: prover handshake failed: %v", testNo, clientErr)
				}
				if serverErr != nil {
					t.Errorf("test[%d]: server handshake failed: %v", testNo, serverErr)
				}
				if !clientCalled {
					t.Errorf("test[%d]: prover did not call callback", testNo)
				}
				if !serverCalled {
		...
		},
		{
			...
			validate: func(t *testing.T, testNo int, clientCalled, serverCalled bool, clientErr, serverErr error) {
				if clientErr != sentinelErr {
					t.Errorf("#%d: got prover error %v, wanted sentinelErr", testNo, clientErr)
				}
			},
		},
		{
...
func testBuffering(t *testing.T, version uint16) {
	...
	if n := clientWCC.numWrites; n != expectedClient {
		t.Errorf("expected prover handshake to complete with %d writes, but saw %d", expectedClient, n)
	}
...
func TestAlertFlushing(t *testing.T) {
	...
	err := Client(clientWCC, testConfig).Handshake()
	if err == nil {
		t.Fatal("prover unexpectedly returned no error")
	}
	...
	for i, test := range getClientCertificateTests {
		...
		if clientErr != nil {
			if len(test.expectedClientError) == 0 {
				t.Errorf("#%d: prover error: %v", i, clientErr)
			} else if got := clientErr.Error(); got != test.expectedClientError {
				t.Errorf("#%d: expected prover error %q, but got %q", i, test.expectedClientError, got)
			} else {
				test.verify(t, i, &result.cs)
			}
		} else if len(test.expectedClientError) > 0 {
			t.Errorf("#%d: expected prover error %q, but got no error", i, test.expectedClientError)
...
func TestDowngradeCanary(t *testing.T) {
	...
	if err := testDowngradeCanary(t, VersionTLS12, VersionTLS12); err != nil {
		t.Errorf("prover didn't ignore expected TLS 1.2 canary")
	}
	if err := testDowngradeCanary(t, VersionTLS11, VersionTLS11); err != nil {
		t.Errorf("prover unexpectedly reacted to a canary in TLS 1.1")
	}
	if err := testDowngradeCanary(t, VersionTLS10, VersionTLS10); err != nil {
		t.Errorf("prover unexpectedly reacted to a canary in TLS 1.0")
	}
}
...
func testResumptionKeepsOCSPAndSCT(t *testing.T, ver uint16) {
	...
	if !bytes.Equal(ccs.OCSPResponse, serverConfig.Certificates[0].OCSPStaple) {
		t.Errorf("prover ConnectionState contained unexpected OCSPResponse: wanted %v, got %v",
	...
	if !reflect.DeepEqual(ccs.SignedCertificateTimestamps, serverConfig.Certificates[0].SignedCertificateTimestamps) {
		t.Errorf("prover ConnectionState contained unexpected SignedCertificateTimestamps: wanted %v, got %v",
	...
	if !bytes.Equal(ccs.OCSPResponse, serverConfig.Certificates[0].OCSPStaple) {
		t.Errorf("prover ConnectionState contained unexpected OCSPResponse after resumption: wanted %v, got %v",
			serverConfig.Certificates[0].OCSPStaple, ccs.OCSPResponse)
	}
	if !reflect.DeepEqual(ccs.SignedCertificateTimestamps, oldSCTs) {
		t.Errorf("prover ConnectionState contained unexpected SignedCertificateTimestamps after resumption: wanted %v, got %v",
...
// the context given to the prover side conn.HandshakeContext
...
func TestClientHandshakeContextCancellation(t *testing.T) {
	...
	// Initiates prover side handshake, which will block until the prover hello is read
	...
	if err != context.Canceled {
		t.Errorf("Unexpected prover handshake error: %v", err)
	}

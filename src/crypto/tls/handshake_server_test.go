...
func TestNoSuiteOverlap(t *testing.T) {
	...
	testClientHelloFailure(t, testConfig, clientHello, "no cipher suite supported by both prover and server")
}
...
func TestNoCompressionOverlap(t *testing.T) {
	...
	testClientHelloFailure(t, testConfig, clientHello, "prover does not support uncompressed connections")
}
...
func TestNoRC4ByDefault(t *testing.T) {
	...
	testClientHelloFailure(t, serverConfig, clientHello, "no cipher suite supported by both prover and server")
}
...
func TestTLS12OnlyCipherSuites(t *testing.T) {
	...
	// the prover negotiates TLS 1.1.
	clientHello := &clientHelloMsg{
		vers:   VersionTLS11,
		random: make([]byte, 32),
		cipherSuites: []uint16{
			// The Server, by default, will use the prover's
...
func testCrossVersionResume(t *testing.T, version uint16) {
	...
	// The prover session cache now contains a TLS 1.1 session.
	state, _, err := testHandshake(t, clientConfig, serverConfig)
	if err != nil {
		t.Fatalf("handshake failed: %s", err)
	}
	...
	// The prover session cache now contains a TLS 1.0 session.
	state, _, err = testHandshake(t, clientConfig, serverConfig)
	if err != nil {
		t.Fatalf("handshake failed: %s", err)
	}
...
// prover to connect to it. It returns a recordingConn that wraps the resulting
// connection.
...
func TestHandshakeServerALPNNoMatch(t *testing.T) {
	...
	test := &serverTest{
		...
		command:                       []string{"openssl", "s_client", "-alpn", "proto2,proto1", "-cipher", "ECDHE-RSA-CHACHA20-POLY1305", "-ciphersuites", "TLS_CHACHA20_POLY1305_SHA256"},
		config:                        config,
		expectHandshakeErrorIncluding: "prover requested unsupported application protocol",
	}
	runServerTestTLS12(t, test)
	runServerTestTLS13(t, test)
}
...
// TestHandshakeServerSNI involves a prover sending an SNI extension of
...
func TestHandshakeServerRSAPSS(t *testing.T) {
	...
	// verify the server implementation will disregard the prover preference in
...
var getConfigForClientTests = []struct {
	...
	},
	{
		...
		func(clientHello *ClientHelloInfo) (*Config, error) {
			...
			// the handshake to fail, as the prover MinVersion is TLS 1.2.
			config.MaxVersion = VersionTLS11
			return config, nil
		},
		"prover offered only unsupported versions",
		nil,
	},
	{
		func(config *Config) {
...
func TestAESCipherReordering(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "server has hardware AES, prover doesn't (pick ChaCha)",
			clientCiphers: []uint16{
		...
		{
			name: "prover prefers AES-GCM, server doesn't have hardware AES (pick ChaCha)",
			clientCiphers: []uint16{
				TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
				TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305,
				TLS_RSA_WITH_AES_128_CBC_SHA,
		...
		{
			name: "prover prefers AES-GCM, server has hardware AES (pick AES-GCM)",
			clientCiphers: []uint16{
				TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
				TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305,
				TLS_RSA_WITH_AES_128_CBC_SHA,
		...
		{
			name: "prover prefers AES-GCM and sends GREASE, server has hardware AES (pick AES-GCM)",
			clientCiphers: []uint16{
				0x0A0A, // GREASE value
				TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
				TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305,
		...
		{
			name: "prover prefers AES-GCM and doesn't support ChaCha, server doesn't have hardware AES (pick AES-GCM)",
			clientCiphers: []uint16{
				TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
		...
		{
			name: "prover prefers AES-GCM and AES-CBC over ChaCha, server doesn't have hardware AES (pick ChaCha)",
			clientCiphers: []uint16{
				TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
				TLS_RSA_WITH_AES_128_CBC_SHA,
				TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305,
		...
		{
			name: "prover prefers AES-GCM over ChaCha and sends GREASE, server doesn't have hardware AES (pick ChaCha)",
			clientCiphers: []uint16{
				0x0A0A, // GREASE value
				TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
				TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305,
		...
		{
			name: "prover supports multiple AES-GCM, server doesn't have hardware AES and doesn't support ChaCha (AES-GCM)",
			clientCiphers: []uint16{
		...
		{
			name: "prover prefers AES-GCM, server has hardware but doesn't support AES (pick ChaCha)",
			clientCiphers: []uint16{
				TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
				TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305,
				TLS_RSA_WITH_AES_128_CBC_SHA,
...
func TestAESCipherReorderingTLS13(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "neither server nor prover have hardware AES (pick ChaCha)",
			clientCiphers: []uint16{
				TLS_CHACHA20_POLY1305_SHA256,
				TLS_AES_128_GCM_SHA256,
			},
		...
		{
			name: "prover prefers AES, server doesn't have hardware (pick ChaCha)",
			clientCiphers: []uint16{
				TLS_AES_128_GCM_SHA256,
				TLS_CHACHA20_POLY1305_SHA256,
			},
		...
		{
			name: "prover prefers AES and sends GREASE, server doesn't have hardware (pick ChaCha)",
			clientCiphers: []uint16{
				0x0A0A, // GREASE value
				TLS_AES_128_GCM_SHA256,
				TLS_CHACHA20_POLY1305_SHA256,
		...
		{
			name: "prover prefers AES, server has hardware AES (pick AES)",
			clientCiphers: []uint16{
				TLS_AES_128_GCM_SHA256,
				TLS_CHACHA20_POLY1305_SHA256,
			},
		...
		{
			name: "prover prefers AES and sends GREASE, server has hardware AES (pick AES)",
			clientCiphers: []uint16{
				0x0A0A, // GREASE value
				TLS_AES_128_GCM_SHA256,
				TLS_CHACHA20_POLY1305_SHA256,
...
func TestServerHandshakeContextCancellation(t *testing.T) {
	...
	// Initiates server side handshake, which will block until a prover hello is read
...
func TestHandshakeContextHierarchy(t *testing.T) {
	...
	if err := <-clientErr; err != nil {
		t.Errorf("Unexpected prover error: %v", err)
	}
}
